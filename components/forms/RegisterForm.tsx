...
const RegisterForm = ({ user }: { user: User }) => {
  ...
  renderSkeleton={(field) => (
    <FormControl>
      <RadioGroup
        className="flex h-11 gap-6 text-white  xl:justify-between"
        onValueChange={(value) => field.onChange(value)}  // Ensure the value is set correctly
        defaultValue={field.value || "Male"}  // Set a default value if needed
      >
        {GenderOptions.map((option, i) => (
          <div key={option + i} className="radio-group">
            <RadioGroupItem value={option} id={option} />
