...
export const LayoutGrid = ({ cards }: { cards: Card[] }) => {
  ...
  return (
    ...
    <div className="relative mx-auto grid size-full max-w-7xl  grid-cols-1 gap-4 pt-2 md:grid-cols-3">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, "")}>
          <motion.div
            onClick={() => handleClick(card)}
  ...
  return (
    <div className="relative z-[60] flex size-full flex-col justify-end rounded-lg bg-transparent shadow-2xl">
      <motion.div
        ...
        className="bg-background absolute inset-0 z-10 size-full opacity-60"
      />
      <motion.div
        layoutId={`content-${selected?.id}`}
        initial={{
